amp <- Event_Segmentation_Custom_ad_totalplay_ad_pause_ad_resume_ad_quartile50_ad_complete_2018_01_01_2018_11_19_

colnames(amp) [1] <- "Event"
colnames(amp) [2] <- "ad-total play"
colnames(amp) [3] <-"ad-pause"
colnames(amp) [4] <-"ad-resume"
colnames(amp) [5] <-"ad-quartile50"
colnames(amp) [6] <-"ad-complete"
...
ampLong <- melt(amp, id=7:17, measure.vars = 2:6)
filtered <- na.omit(ampLong)
filtered$total<- rowSums(filtered[,1:11])
filtered$product <- "native-podcast"
...
write.table(filtered, file = "native_(11.19).csv", row.names = FALSE, sep = ",")
